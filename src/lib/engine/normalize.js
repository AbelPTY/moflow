...
export function processTransactionRow(t, options = {}) {
  ...
  const description = (t.description_raw || t.description || '').toUpperCase();

  // Prevent account masks (like **** 3355) from becoming the merchant name
  let merchantName = (t.merchant_extracted && t.merchant_extracted.length > 1 && !t.merchant_extracted.includes('****'))
  ...
  const match = classifyTransaction({ merchant: merchantName, description, amount: rawAmount }, rulesData?.rules, userRules);
  if (match) {
    ruleMatched = true;
    ...
    if (it !== null && it !== undefined) is_transfer = it;

    if (match.kind === 'manual' || match.kind === 'migrated' || match.kind === 'fallback') {
    ...
    if ((match.kind === 'static' || match.kind === 'migrated') && rawAmount > 0 && ['Sports', 'Office/Social Events', 'Work Expenses'].includes(category)) {
  ...
  if (!userRuleMatched) {
    if (['TRANSFERS', 'CC_PAYMENT', 'ADJUSTMENT'].includes(budgetBucket)) { is_transfer = true; }
    else if (['INCOME', 'NEEDS', 'WANTS', 'SAVINGS'].includes(budgetBucket)) { is_transfer = false; }
  }
  ...
  return {
      ...
      account: t.source_account || t.account_name,
